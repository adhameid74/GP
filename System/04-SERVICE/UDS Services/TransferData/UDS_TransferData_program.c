...
u16 TransferData_u8NextAddress = EEPROM_APP_START_ADDRESS;
...
void UDS_voidTransferData(INDICATION_SDU* ReceivedMessage)
{
    ...
    if( (ReceivedMessage->Length.u12 < 3) || (ReceivedMessage->Length.u12 > maxNumberOfBlockLength) )
    {
    ...
    {
        if ( (RequestDownload_u32Size - TransferData_u32ReceivedBytes) < (ReceivedMessage->Length.u12-2) )
        {
        ...
        for (u8 Local_u8Counter = 3; Local_u8Counter < (ReceivedMessage->Length.u12-2); Local_u8Counter+=2)
        {
        ...
        Local_u8Byte = ReceivedMessage->MessageData[ReceivedMessage->Length.u12-2] << 4;
        Local_u8Byte |= (ReceivedMessage->MessageData[ReceivedMessage->Length.u12-1] & 0x0F);
        ...
        TransferData_u32ReceivedBytes+=(ReceivedMessage->Length.u12-2);
